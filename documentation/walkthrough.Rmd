...
subtitle: For epidemia-demo v.1.0.0 using epidemiar v1.1.1
author: |
...
If you have not installed `epidemiar`, MikTeX, or other components yet, please see the Install & Update Guide (`"documentation/install_update.pdf"`) for details on those steps. 
...
This is the regression object from the general additive model (GAM, parallelized with BAM) regression. This is only for statistical investigation of the model, and is usually not saved because it very large object.
...
(Note: there is a `show_report` argument that can be turned off, which is especially useful if you are writing loops to generate many reports sequentially.)
