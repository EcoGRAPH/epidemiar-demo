...
subtitle: For epidemia-demo v3.0 using epidemiar v3.1.0
output:
  pdf_document:
    number_sections: yes
    toc: yes
...
Open the `epidemiar_demo.Rproj`. From here, there are three scripts of major importance. 
...
3. The parameter file `data/epidemiar_settings_amhara.R` contains the common parameters for both scripts. 
...
We do not use any transformation of the case counts for modeling, but there is an option for a "log_plus_one" transformation (and back-transformation), if relevant for certain model families. 
...
#fc_splines <- "modbs" #modified b-splines, faster but tends to over-fit more than thin plate
...
The processing time depends on how powerful your computer is and the settings used for modeling. Passing in a model reduces the time, since it does not have to calculate the model first.  Expect this to take about 3 to 10 minutes per species on an average computer. 
...
if (exists("pfm_reportdata") & exists("pv_reportdata")){
  ...
  merge_save_report(rpt_data_main = pfm_reportdata,
                    ...
                    #save out a second copy of the report data with year & week numbers in the name
                    second_save = TRUE,
                    #create the pdf
                    create_report = TRUE,
