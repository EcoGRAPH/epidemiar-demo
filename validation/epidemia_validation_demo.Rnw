...
\vspace{\fill}
\noindent Skill scores show the improvement (with values greater than 0) of the forecast model (\Sexpr{val$metadata$fc_model_family}) over simple naive models. There are three accuracy statistics used: Mean Absolute Error (MAE), Root Mean Squared Error (RMSE), an a R2 (variance explained). These statstics are compared against the naive models to give the skill scores. The boxplot shows the concordance of skill among the three statistics. Higher values (0 to 1) indicate the greater skill of the forecast models over the simple naive models. 
